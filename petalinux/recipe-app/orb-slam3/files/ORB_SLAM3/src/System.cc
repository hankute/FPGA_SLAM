...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // ------------------ FPS 统计（基于实际处理时间 - 每秒打印一次） ------------------
    // 静态变量，跨函数调用保留状态
    static int s_frame_count = 0;
    static std::chrono::steady_clock::time_point s_last_fpstime = std::chrono::steady_clock::now();
    static std::mutex s_fps_mutex;
    static int flag=0;
    {
        std::lock_guard<std::mutex> lock(s_fps_mutex);
        ++s_frame_count;
        auto now = std::chrono::steady_clock::now();
        double elapsed = std::chrono::duration_cast<std::chrono::duration<double>>(now - s_last_fpstime).count();
        if (elapsed >= 1.0) {
            double fps = static_cast<double>(s_frame_count) / elapsed;
            std::cout << "[System] TrackStereo FPS: " << fps
                      << "  (frames=" << s_frame_count << ", elapsed=" << elapsed << "s)"
                      << "  timestamp=" << timestamp
                      << std::endl;
            s_frame_count = 0;
            s_last_fpstime = now;
            if(flag==0){
                general_fps=general_fps+fps;
                flag=1;
            }
            else{
                general_fps=(general_fps+fps)/2.0;
            }
        }
    }
...
void System::Shutdown()
{
    ...
    mpTracker->PrintTimeStats();
        ...
        std::cout << "[System] Average (processing) FPS: " << general_fps << std::endl;
}
